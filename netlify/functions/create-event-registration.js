...
const main = async (req) => {
  try {
    ...
    if (!event_slug || !course_name) return json({ error: 'Campos obrigatórios: event_slug e course_name.' }, 400)

    const user = await getUserById(userId)
    if (!user) return json({ error: 'Somente usuários cadastrados podem se inscrever.' }, 403)
    if (user.status !== 'ativo') return json({ error: 'Usuário inativo.' }, 403)
    if (user.perfil !== 'autor') return json({ error: 'A inscrição é permitida apenas para usuários com perfil de autor.' }, 403)
