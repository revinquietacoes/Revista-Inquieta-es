...
const CHIEF_ALLOWED_SUBMISSION_STATUSES = [
  'submetido',
  'em_avaliacao',
  'rejeitado',
...
async function getChiefSubmissionStatusQueue() {
  const rows = await sql`
    SELECT s.id, s.titulo, s.status, s.secao, s.data_submissao, s.status_atualizado_em,
           s.status_atualizado_por, a.nome AS autor_nome, dt.titulo AS dossie_titulo,
...
async function updateChiefSubmissionStatus(payload, user) {
  ...
  await sql`
    UPDATE submissoes
    SET status = ${novoStatus}, status_atualizado_em = CURRENT_TIMESTAMP, status_atualizado_por = ${user.id}
  ...
  await sql`
    ...
    VALUES (${submissaoId}, ${atual[0].status}, ${novoStatus}, ${observacao}, ${user.id})`
...
async function updateDesignacaoStatus(payload, user) {
  ...
  if (user.perfil === 'parecerista') {
    rows = await sql`
      ...
      WHERE id = ${id} AND parecerista_id = ${user.id}
      RETURNING id, status`
  } else {
    rows = await sql`
      UPDATE designacoes_avaliacao
...
async function getReviewerReviewHistory(user, reviewerId) {
  ...
  let allowedReviewer = []
  if (user.perfil === 'editor_chefe' || user.perfil === 'editor' || user.perfil === 'editor_adjunto') {
  ...
  try {
    ...
    if (action === 'editor_dashboard') {
      ...
      const pareceristas = await sql`
        SELECT u.id, u.nome, u.email, u.instituicao, u.orcid, u.lattes, u.foto_perfil_url,
