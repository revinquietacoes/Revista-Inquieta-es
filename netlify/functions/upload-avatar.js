...
const main = async (req) => {
  try {
    ...
    const antigos = await sql`
      SELECT id, blob_key FROM arquivos_publicacao
      WHERE usuario_id = ${usuarioId} AND categoria = 'foto_perfil'
      ORDER BY id DESC`
    ...
    await store.set(blobKey, bytes, { metadata: { usuario_id: String(usuarioId), categoria: 'foto_perfil', nome_original: arquivo.name, mime_type: arquivo.type } })
