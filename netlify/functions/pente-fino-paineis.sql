-- Migração de pente-fino para os painéis da Revista Inquietações
-- Pode ser executada no Neon/Postgres com segurança (idempotente na maior parte dos pontos).
...
BEGIN
  ...
  DO UPDATE SET total_decisoes_editoriais = contribuicoes_usuarios.total_decisoes_editoriais + 1,
                atualizado_em = CURRENT_TIMESTAMP;
END;
$$;
...
ORDER BY COALESCE(av2.atualizado_em, av.atualizado_em, da.atualizado_em, da.criado_em) DESC NULLS LAST;

INSERT INTO contribuicoes_usuarios (usuario_id, total_submissoes)
SELECT autor_id, COUNT(*)
FROM submissoes
WHERE autor_id IS NOT NULL
GROUP BY autor_id
ON CONFLICT (usuario_id)
DO UPDATE SET total_submissoes = EXCLUDED.total_submissoes,
              atualizado_em = CURRENT_TIMESTAMP;

INSERT INTO contribuicoes_usuarios (usuario_id, total_avaliacoes)
SELECT parecerista_id, COUNT(*)
FROM designacoes_avaliacao
WHERE parecerista_id IS NOT NULL AND status = 'concluido'
GROUP BY parecerista_id
ON CONFLICT (usuario_id)
DO UPDATE SET total_avaliacoes = EXCLUDED.total_avaliacoes,
              atualizado_em = CURRENT_TIMESTAMP;

INSERT INTO contribuicoes_usuarios (usuario_id, total_dossies)
SELECT editor_responsavel_id, COUNT(*)
FROM dossies_tematicos
WHERE editor_responsavel_id IS NOT NULL
GROUP BY editor_responsavel_id
ON CONFLICT (usuario_id)
DO UPDATE SET total_dossies = EXCLUDED.total_dossies,
              atualizado_em = CURRENT_TIMESTAMP;
