...
const main = async (req) => {
  try {
    ...
    let buffer
    try { buffer = Buffer.from(file_base64, 'base64') } catch { return json({ erro: 'Arquivo em base64 inválido.' }, 400) }
    if (!buffer || !buffer.length) return json({ erro: 'Arquivo vazio ou inválido.' }, 400)

    await certificatesStore.set(blobKey, buffer, { metadata: { title, tipo: certificate_type, target_user_id: String(targetUser.id), uploaded_by: String(editorId), mime_type: mime_type || 'application/pdf' } })
