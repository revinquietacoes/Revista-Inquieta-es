...
const main = async (req) => {
  try {
    ...
    if (cols.has('instituicao')) {
      campos.push('instituicao')
      valores.push(instituicao || null)
    }
    if (cols.has('orcid')) {
      campos.push('orcid')
      valores.push(orcid || null)
    }
    if (cols.has('lattes')) {
      campos.push('lattes')
      valores.push(lattes || null)
    }
    if (cols.has('origem')) {
      campos.push('origem')
    ...
    if (cols.has('foto_perfil_url')) {
      campos.push('foto_perfil_url')
      valores.push(foto_perfil_url || 'assets/avatares/avatar-padrao.png')
    }
    if (cols.has('foto_perfil_aprovada')) {
      campos.push('foto_perfil_aprovada')
      valores.push(false)
    }
    if (cols.has('consentimento_foto_publica')) {
      campos.push('consentimento_foto_publica')
      valores.push(Boolean(consentimento_foto_publica))
    }
    if (cols.has('receber_noticias_email')) {
      campos.push('receber_noticias_email')
      valores.push(Boolean(receber_noticias_email))
    }
    if (cols.has('status')) {
      campos.push('status')
      valores.push(statusInicial)
    }
